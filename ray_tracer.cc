...
void process_bar(int j, int image_height) {
  double percent = 1 - (double)j / image_height;
  int num = static_cast<int>(percent * 50);
  std::cerr << "\r[";
  for (int l = 0; l < 50; l++) {
    if (l == num - 1)
      std::cerr << ">";
...
hittable_list two_perlin_spheres() {
  ...
  objects.add(make_shared<sphere>(point3(0, -1000.0, 0), 1000,
